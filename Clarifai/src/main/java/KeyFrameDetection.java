...
//        VideoDisplay<MBFImage> display = VideoDisplay.createVideoDisplay(video);
    ...
    public static void MainFrames(){
        ...
        Double max = Collections.max(mainPoints); //max keypoints size we are collecting. Normalizing relaition
        for(int i=0; i<mainPoints.size(); i++){
            if(((mainPoints.get(i))/max < 0.01) || i==0){  //if a less than 0.1 then it is keyframe
                //if say
